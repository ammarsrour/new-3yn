    ...
    <section id="features" className="py-32 bg-white">
      <div className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-12">
        ...
        <div className="grid lg:grid-cols-12 gap-8 lg:gap-6">
          ...
          <div className={`lg:col-span-7 bg-navy-950 p-10 lg:p-14 ${isArabic ? 'text-right' : ''}`}>
            <div className="flex items-start justify-between mb-8">
              <Zap className={`w-10 h-10 text-emerald-400 ${isArabic ? 'order-2' : ''}`} />
              <span className="text-emerald-400 text-6xl lg:text-7xl text-stat tabular-nums">{features[0].stat}</span>
            </div>
          ...
          <div className={`lg:col-span-5 bg-slate-100 p-10 lg:p-14 ${isArabic ? 'text-right' : ''}`}>
            <div className="flex items-start justify-between mb-8">
              <Target className={`w-8 h-8 text-navy-950 ${isArabic ? 'order-2' : ''}`} />
              <span className="text-navy-950 text-5xl text-stat tabular-nums">{features[1].stat}</span>
            </div>
          ...
          <div className={`lg:col-span-5 border-2 border-slate-200 p-10 ${isArabic ? 'text-right' : ''}`}>
            <div className="flex items-start justify-between mb-6">
              <FileText className={`w-8 h-8 text-navy-950 ${isArabic ? 'order-2' : ''}`} />
              <span className="text-emerald-600 text-4xl text-stat tabular-nums">{features[2].stat}</span>
            </div>
          ...
          <div className={`lg:col-span-7 border-2 border-slate-200 p-10 ${isArabic ? 'text-right' : ''}`}>
            <div className="flex items-start justify-between mb-6">
              <MapPin className={`w-8 h-8 text-navy-950 ${isArabic ? 'order-2' : ''}`} />
              <span className="text-emerald-600 text-4xl text-stat tabular-nums">{features[3].stat}</span>
            </div>
